...
def hourglass(window, n, point, radius, color):
    ...
    # -------------------------------------------------------------------------
    circle3=rg.Circle(point,radius)
    circle3.attach_to(window)
    for k in range (n):
        circle=rg.Circle(rg.Point(point.x-(radius*k),point.y-(math.sqrt((2*radius)**2-radius**2))*k),radius)

        for i in range (k+1):
            circle2=rg.Circle(rg.Point(circle.center.x+radius*i*2,circle.center.y),radius)
            circle2.fill_color=color
            circle2.attach_to(window)
            line=rg.Line(rg.Point(circle2.center.x-radius,circle2.center.y),rg.Point(circle2.center.x+radius,circle2.center.y))
...
def many_hourglasses(window, square, m, colors):
    ...
    point3=rg.Point(square.center.x,square.center.y)
    radius=square.length_of_each_side/2
    for k in range (m):
        rectangle=rg.Rectangle(point1,point2)
        rectangle.attach_to(window)
        hourglass(window,k+1,point3,radius,colors[k % len(colors)])
        point3.x=point3.x +(k+3/2)*square.length_of_each_side
        point1.x=point2.x
        point1.y=point1.y-((2*radius)**2-radius**2)**(1/2)
        point2.x=point2.x + (k+2)*square.length_of_each_side
        point2.y=point2.y+((2*radius)**2-radius**2)**(1/2)
    window.render()
